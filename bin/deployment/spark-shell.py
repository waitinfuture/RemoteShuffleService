...
def main(argv):
    ...
    if master_address == '':
        ...
        os.system('spark-shell ' + ' '.join(argv[1:]))
    else:
        print("run spark with ESS")
        os.system('spark-shell --conf spark.shuffle.manager=org.apache.spark.shuffle.ess.EssShuffleManager --conf spark.ess.master.address=' + master_address + ' ' + ' '.join(argv[1:]))
