...
class EssConf(loadDefaults: Boolean) extends Cloneable with Logging with Serializable {
  ...
  @transient private lazy val reader: ConfigReader = {
    val _reader = new ConfigReader(new JindoConfigProvider(settings))
  ...
  /**
    ...
    */
  private[jss] def get[T](entry: ConfigEntry[T]): T = {
  ...
  private[jss] def getWithSubstitution(key: String): Option[String] = {
  ...
  /**
    ...
    * in unit tests.
    */
  private[jss] def getenv(name: String): String = System.getenv(name)
...
private[jss] object EssConf extends Logging {
  ...
  def logDeprecationWarning(key: String): Unit = {
    deprecatedConfigs.get(key).foreach { cfg =>
      logWarning(
        s"The configuration key '$key' has been deprecated as of Jindo ${cfg.version} and " +
