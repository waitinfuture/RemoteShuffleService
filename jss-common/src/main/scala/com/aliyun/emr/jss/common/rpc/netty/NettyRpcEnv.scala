...
private[jss] class NettyRpcEnv(
  ...
  private[jss] val clientConnectionExecutor = ThreadUtils.newDaemonCachedThreadPool(
   ...
   */
  private[jss] def removeOutbox(address: RpcAddress): Unit = {
  ...
  private[jss] def send(message: RequestMessage): Unit = {
  ...
  private[jss] def createClient(address: RpcAddress): TransportClient = {
    clientFactory.createClient(address.host, address.port)
  }

  private[jss] def ask[T: ClassTag](message: RequestMessage, timeout: RpcTimeout): Future[T] = {
  ...
  private[jss] def pushData[T: ClassTag](pushData: PushData,
  ...
  private[jss] def serialize(content: Any): ByteBuffer = {
   ...
   */
  private[jss] def serializeStream(out: OutputStream): SerializationStream = {
    javaSerializerInstance.serializeStream(out)
  }

  private[jss] def deserialize[T: ClassTag](client: TransportClient, bytes: ByteBuffer): T = {
...
private[jss] object NettyRpcEnv extends Logging {
  /**
   ...
   * }}}
   */
  private[jss] val currentEnv = new DynamicVariable[NettyRpcEnv](null)
   ...
   */
  private[jss] val currentClient = new DynamicVariable[TransportClient](null)
 ...
 */
private[jss] class RequestMessage(
...
private[jss] object RequestMessage {
 ...
 */
private[jss] case class RpcFailure(e: Throwable)
 ...
 */
private[jss] class NettyRpcHandler(
