...
private[ess] class NettyRpcEnv(
  ...
  private[ess] val clientConnectionExecutor = ThreadUtils.newDaemonCachedThreadPool(
   ...
   */
  private[ess] def removeOutbox(address: RpcAddress): Unit = {
  ...
  private[ess] def send(message: RequestMessage): Unit = {
  ...
  private[ess] def createClient(address: RpcAddress): TransportClient = {
    clientFactory.createClient(address.host, address.port)
  }

  private[ess] def ask[T: ClassTag](message: RequestMessage, timeout: RpcTimeout): Future[T] = {
  ...
  private[ess] def serialize(content: Any): ByteBuffer = {
   ...
   */
  private[ess] def serializeStream(out: OutputStream): SerializationStream = {
    javaSerializerInstance.serializeStream(out)
  }

  private[ess] def deserialize[T: ClassTag](client: TransportClient, bytes: ByteBuffer): T = {
  ...
  private def downloadClient(host: String, port: Int): TransportClient = {
    if (fileDownloadFactory == null) synchronized {
      if (fileDownloadFactory == null) {
        ...
        val downloadConf = Utils.fromEssConf(clone, module, ioThreads)
...
private[ess] object NettyRpcEnv extends Logging {
  /**
   ...
   * }}}
   */
  private[ess] val currentEnv = new DynamicVariable[NettyRpcEnv](null)
   ...
   */
  private[ess] val currentClient = new DynamicVariable[TransportClient](null)

}

private[ess] class NettyRpcEnvFactory extends RpcEnvFactory with Logging {
 ...
 */
private[ess] class NettyRpcEndpointRef(
 ...
 */
private[ess] class RequestMessage(
...
private[ess] object RequestMessage {
 ...
 */
private[ess] case class RpcFailure(e: Throwable)
 ...
 */
private[ess] class NettyRpcHandler(
