 ...
 */
private[ess] case class RpcEndpointAddress(rpcAddress: RpcAddress, name: String) {
  ...
  override val toString = if (rpcAddress != null) {
      s"ess://$name@${rpcAddress.host}:${rpcAddress.port}"
    } else {
      s"ess-client://$name"
    }
}

private[ess] object RpcEndpointAddress {
  ...
  def apply(essUrl: String): RpcEndpointAddress = {
    try {
      ...
      if (uri.getScheme != "ess" ||
          ...
          uri.getQuery != null) {
        throw new EssException("Invalid ESS URL: " + essUrl)
      }
      new RpcEndpointAddress(host, port, name)
    } catch {
      case e: java.net.URISyntaxException =>
        throw new EssException("Invalid ESS URL: " + essUrl, e)
    }
  }
}
