...
private[ess] case class OneWayOutboxMessage(content: ByteBuffer) extends OutboxMessage
  with Logging {
...
private[ess] case class RpcOutboxMessage(
    content: ByteBuffer,
...
private[ess] class Outbox(nettyEnv: NettyRpcEnv, val address: RpcAddress) {
  ...
  def send(message: OutboxMessage): Unit = {
    ...
    if (dropped) {
      message.onFailure(new EssException("Message is dropped because Outbox is stopped"))
    } else {
      drainOutbox()
    }
  }
