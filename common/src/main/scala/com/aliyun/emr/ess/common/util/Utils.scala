...
object Utils
  ...
  @throws(classOf[EssException])
  def extractHostPortFromEssUrl(essUrl: String): (String, Int) = {
    try {
      val uri = new java.net.URI(essUrl)
      val host = uri.getHost
      val port = uri.getPort
      if (uri.getScheme != "ess" ||
        host == null ||
        port < 0 ||
        ...
        throw new EssException("Invalid master URL: " + essUrl)
      }
      (host, port)
    } catch {
      case e: java.net.URISyntaxException =>
        throw new EssException("Invalid master URL: " + essUrl, e)
    }
  }
  ...
  def startServiceOnPort[T](
    ...
    // Should never happen
    throw new EssException(s"Failed to start service$serviceString on port $startPort")
  }
  ...
  def fromEssConf(_conf: EssConf, module: String, numUsableCores: Int = 0): TransportConf = {
    ...
    conf.setIfMissing(s"ess.$module.io.serverThreads", numThreads.toString)
    conf.setIfMissing(s"ess.$module.io.clientThreads", numThreads.toString)
  ...
  def getPropertiesFromFile(filename: String): Map[String, String] = {
    ...
    try {
      ...
      case e: IOException =>
        throw new EssException(s"Failed when loading ESS properties from $filename", e)
    } finally {
      inReader.close()
    }
  }
