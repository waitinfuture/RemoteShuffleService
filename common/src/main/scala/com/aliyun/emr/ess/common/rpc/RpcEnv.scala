...
import com.aliyun.emr.ess.common.rpc.netty.NettyRpcEnvFactory
import com.aliyun.emr.ess.common.util.RpcUtils
 ...
 */
private[ess] abstract class RpcEnv(conf: EssConf) {

  private[ess] val defaultLookupTimeout = RpcUtils.lookupRpcTimeout(conf)

  /**
   * Return RpcEndpointRef of the registered [[RpcEndpoint]]. Will be used to implement
   * [[RpcEndpoint.self]]. Return `null` if the corresponding [[RpcEndpointRef]] does not exist.
   */
  private[ess] def endpointRef(endpoint: RpcEndpoint): RpcEndpointRef
 ...
 */
private[ess] trait RpcEnvFileServer {
...
private[ess] case class RpcEnvConfig(
    conf: EssConf,
    name: String,
    bindAddress: String,
    advertiseAddress: String,
