...
import com.aliyun.rss.common.network.client.{RpcResponseCallback, TransportClient}
import com.aliyun.rss.common.rpc.{RpcAddress, RpcEnvStoppedException}
...
private[rss] class Outbox(nettyEnv: NettyRpcEnv, val address: RpcAddress) {
   ...
   * [[Outbox]] is stopped, the sender will be notified with a [[RssException]].
   */
  def send(message: OutboxMessage): Unit = {
    ...
    if (dropped) {
      message.onFailure(new RssException("Message is dropped because Outbox is stopped"))
    } else {
      drainOutbox()
    }
  }
