...
package com.aliyun.rss.common.rpc.netty
...
import com.aliyun.rss.common.network.TransportContext
import com.aliyun.rss.common.network.client._
import com.aliyun.rss.common.network.server._
import com.aliyun.rss.common.protocol.{RpcNameConstants, TransportModuleConstants};
import com.aliyun.rss.common.rpc._
import com.aliyun.rss.common.serializer.{JavaSerializer, JavaSerializerInstance, SerializationStream}
import com.aliyun.rss.common.util.{ByteBufferInputStream, ByteBufferOutputStream, ThreadUtils, Utils}
...
class NettyRpcEnv(
  ...
  private[rss] val clientConnectionExecutor = ThreadUtils.newDaemonCachedThreadPool(
   ...
   */
  private[rss] def removeOutbox(address: RpcAddress): Unit = {
  ...
  private[rss] def send(message: RequestMessage): Unit = {
  ...
  private[rss] def createClient(address: RpcAddress): TransportClient = {
    clientFactory.createClient(address.host, address.port)
  }

  private[rss] def ask[T: ClassTag](message: RequestMessage, timeout: RpcTimeout): Future[T] = {
  ...
  private[rss] def serialize(content: Any): ByteBuffer = {
   ...
   */
  private[rss] def serializeStream(out: OutputStream): SerializationStream = {
    javaSerializerInstance.serializeStream(out)
  }

  private[rss] def deserialize[T: ClassTag](client: TransportClient, bytes: ByteBuffer): T = {
...
private[rss] object NettyRpcEnv extends Logging {
   ...
   * }}}
   */
  private[rss] val currentEnv = new DynamicVariable[NettyRpcEnv](null)
   ...
   */
  private[rss] val currentClient = new DynamicVariable[TransportClient](null)

}

private[rss] class NettyRpcEnvFactory extends RpcEnvFactory with Logging {

  def create(config: RpcEnvConfig): RpcEnv = {
    val essConf = config.conf
    // Use JavaSerializerInstance in multiple threads is safe. However, if we plan to support
    // KryoSerializer in future, we have to use ThreadLocal to store SerializerInstance
    val javaSerializerInstance =
      new JavaSerializer(essConf).newInstance().asInstanceOf[JavaSerializerInstance]
    val nettyEnv =
      new NettyRpcEnv(essConf, javaSerializerInstance, config.advertiseAddress,
    ...
    try {
      Utils.startServiceOnPort(config.port, startNettyRpcEnv, essConf, config.name)._1
    } catch {
      case NonFatal(e) =>
        nettyEnv.shutdown()
        throw e
 ...
 */
private[rss] class RequestMessage(
...
private[rss] object RequestMessage {
 ...
 */
private[rss] case class RpcFailure(e: Throwable)
