...
package com.aliyun.emr.network.util;
...
public class TransportConf {
  ...
  private final String ESS_NETWORK_IO_SERVERTHREADS_KEY;
  private final String ESS_NETWORK_IO_CLIENTTHREADS_KEY;
  private final String ESS_NETWORK_IO_RECEIVEBUFFER_KEY;
  private final String ESS_NETWORK_IO_SENDBUFFER_KEY;
  private final String ESS_NETWORK_SASL_TIMEOUT_KEY;
  private final String ESS_NETWORK_IO_MAXRETRIES_KEY;
  private final String ESS_NETWORK_IO_RETRYWAIT_KEY;
  private final String ESS_NETWORK_IO_LAZYFD_KEY;
  private final String ESS_NETWORK_VERBOSE_METRICS;
  ...
  public TransportConf(String module, ConfigProvider conf) {
    ...
    ESS_NETWORK_IO_SERVERTHREADS_KEY = getConfKey("io.serverThreads");
    ESS_NETWORK_IO_CLIENTTHREADS_KEY = getConfKey("io.clientThreads");
    ESS_NETWORK_IO_RECEIVEBUFFER_KEY = getConfKey("io.receiveBuffer");
    ESS_NETWORK_IO_SENDBUFFER_KEY = getConfKey("io.sendBuffer");
    ESS_NETWORK_SASL_TIMEOUT_KEY = getConfKey("sasl.timeout");
    ESS_NETWORK_IO_MAXRETRIES_KEY = getConfKey("io.maxRetries");
    ESS_NETWORK_IO_RETRYWAIT_KEY = getConfKey("io.retryWait");
    ESS_NETWORK_IO_LAZYFD_KEY = getConfKey("io.lazyFD");
    ESS_NETWORK_VERBOSE_METRICS = getConfKey("io.enableVerboseMetrics");
  }
  ...
  public String ioMode() {
    return conf.get(ESS_NETWORK_IO_MODE_KEY, "NIO").toUpperCase(Locale.ROOT);
  }
  ...
  public int numConnectionsPerPeer() {
    return conf.getInt(ESS_NETWORK_IO_NUMCONNECTIONSPERPEER_KEY, 16);
  }

  /** Requested maximum length of the queue of incoming connections. Default -1 for no backlog. */
  public int backLog() { return conf.getInt(ESS_NETWORK_IO_BACKLOG_KEY, -1); }

  /** Number of threads used in the server thread pool. Default to 0, which is 2x#cores. */
  public int serverThreads() { return conf.getInt(ESS_NETWORK_IO_SERVERTHREADS_KEY, 0); }

  /** Number of threads used in the client thread pool. Default to 0, which is 2x#cores. */
  public int clientThreads() { return conf.getInt(ESS_NETWORK_IO_CLIENTTHREADS_KEY, 0); }
   ...
   */
  public int receiveBuf() { return conf.getInt(ESS_NETWORK_IO_RECEIVEBUFFER_KEY, -1); }

  /** Send buffer size (SO_SNDBUF). */
  public int sendBuf() { return conf.getInt(ESS_NETWORK_IO_SENDBUFFER_KEY, -1); }

  /** Timeout for a single round trip of auth message exchange, in milliseconds. */
  public int authRTTimeoutMs() {
    return (int) JavaUtils.timeStringAsSec(conf.get("ess.network.auth.rpcTimeout",
      conf.get(ESS_NETWORK_SASL_TIMEOUT_KEY, "30s"))) * 1000;
  }
   ...
   */
  public int maxIORetries() { return conf.getInt(ESS_NETWORK_IO_MAXRETRIES_KEY, 3); }
  ...
  public int ioRetryWaitTimeMs() {
    return (int) JavaUtils.timeStringAsSec(conf.get(ESS_NETWORK_IO_RETRYWAIT_KEY, "5s")) * 1000;
  }
  ...
  public int memoryMapBytes() {
    return Ints.checkedCast(JavaUtils.byteStringAsBytes(
      conf.get("ess.storage.memoryMapThreshold", "2m")));
  }
  ...
  public boolean lazyFileDescriptor() {
    return conf.getBoolean(ESS_NETWORK_IO_LAZYFD_KEY, true);
  }
  ...
  public boolean verboseMetrics() {
    return conf.getBoolean(ESS_NETWORK_VERBOSE_METRICS, false);
  }
  ...
  public int portMaxRetries() {
    return conf.getInt("ess.port.maxRetries", 16);
  }
  ...
  public boolean encryptionEnabled() {
    return conf.getBoolean("ess.network.crypto.enabled", false);
  }
  ...
  public String cipherTransformation() {
    return conf.get("ess.network.crypto.cipher", "AES/CTR/NoPadding");
  }
  ...
  public String keyFactoryAlgorithm() {
    return conf.get("ess.network.crypto.keyFactoryAlgorithm", "PBKDF2WithHmacSHA1");
  }
  ...
  public int keyFactoryIterations() {
    return conf.getInt("ess.network.crypto.keyFactoryIterations", 1024);
  }
  ...
  public int encryptionKeyLength() {
    return conf.getInt("ess.network.crypto.keyLength", 128);
  }
   ...
   */
  public int ivLength() {
    return conf.getInt("ess.network.crypto.ivLength", 16);
  }
  ...
  public String keyAlgorithm() {
    return conf.get("ess.network.crypto.keyAlgorithm", "AES");
  }
  ...
  public boolean saslFallback() {
    return conf.getBoolean("ess.network.crypto.saslFallback", true);
  }
  ...
  public boolean saslEncryption() {
    return conf.getBoolean("ess.authenticate.enableSaslEncryption", false);
  }
  ...
  public int maxSaslEncryptedBlockSize() {
    return Ints.checkedCast(JavaUtils.byteStringAsBytes(
      conf.get("ess.network.sasl.maxEncryptedBlockSize", "64k")));
  }
  ...
  public boolean saslServerAlwaysEncrypt() {
    return conf.getBoolean("ess.network.sasl.serverAlwaysEncrypt", false);
  }
  ...
  public Properties cryptoConf() {
    return CryptoUtils.toCryptoConf("ess.network.crypto.config.", conf.getAll());
  }
   ...
   * retry according to the shuffle retry configs (see `ess.shuffle.io.maxRetries` and
   * `ess.shuffle.io.retryWait`), if those limits are reached the task will fail with fetch
   * failure.
   */
  public long maxChunksBeingTransferred() {
    return conf.getLong("ess.shuffle.maxChunksBeingTransferred", Long.MAX_VALUE);
  }
}
