package com.aliyun.emr.ess.service.deploy
...
import com.aliyun.emr.ess.protocol.message.DataMessages.{GetDoubleChunkInfo, GetDoubleChunkInfoResponse, PushDataResponse}
...
class MessageHandlerSuite
  ...
  def init(numWorkers: Int = 2) {
    ...
    conf.set("ess.worker.base.dir", EssConf.TEST_HDFS_BASE)
    ...
    val masterAddresses: RpcAddress = RpcAddress.fromEssURL(workerArgs.master)
    rpcEnvWorker1.setupEndpoint(RpcNameConstants.WORKER_EP,
      new Worker(rpcEnvWorker1,
  ...
  test("PushData") {
    ...
    0 until array.length foreach (ind => array(ind) = 'a')
    val buf = new NettyManagedBuffer(Unpooled.wrappedBuffer(array))
    ...
    var res1 = worker1.askSync[PushDataResponse](pushDataMsg1)
    assert(res1.status == StatusCode.Success)
    res1 = worker1.askSync[PushDataResponse](pushDataMsg1)
    assert(res1.status == StatusCode.Success)
    ...
    var res = worker2.askSync[PushDataResponse](pushDataMsg2)
    assert(res.status == StatusCode.Success)
    waitUntilDataFinishFlushing(worker2, shuffleKey)
    assert(getFileLengh(shuffleKey, worker2Location.getReduceId, worker2Location.getUUID) == 0)
    res = worker2.askSync[PushDataResponse](pushDataMsg2)
    assert(res.status == StatusCode.Success)
    waitUntilDataFinishFlushing(worker2, shuffleKey)
    assert(getFileLengh(shuffleKey, worker2Location.getReduceId, worker2Location.getUUID) == 0)
    res = worker2.askSync[PushDataResponse](pushDataMsg2)
    ...
    res = worker2.askSync[PushDataResponse](pushDataMsg3)
    assert(res.status == StatusCode.Success)
    waitUntilDataFinishFlushing(worker2, shuffleKey)
  ...
  test("SlaveLost") {
    ...
    0 until 5 foreach (_ => {
      ...
      worker1.askSync[PushDataResponse](
        new PushData(
          PartitionLocation.Mode.Master.mode(),
          shuffleKey,
  ...
  test("WorkerLost-3Workers") {
    ...
    0 until 5 foreach (_ => {
      resReg.partitionLocations.foreach(loc => {
        ...
        Random.nextBytes(data)
        val buf = new NettyManagedBuffer(Unpooled.wrappedBuffer(data))
        val res = getWorker(loc).askSync[PushDataResponse](
          new PushData(
            PartitionLocation.Mode.Master.mode,
            shuffleKey,
            loc.getUUID,
  ...
  test("ReadPartition") {
    ...
    0 until 10 foreach(reduceId => {
      0 until 10 foreach(mapId => {
        ...
        println
        client.pushData(appId, shuffleId, mapId, 0, reduceId, data)
      })
    })
