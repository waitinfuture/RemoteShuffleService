...
class ClusterReadWriteTest extends MiniClusterFeature {
  ...
  def testMiniCluster(): Unit = {
    ...
    val dataSize1 = shuffleClient.pushData(APP, 1, 0, 0, 0, DATA1,1, 1)
    ...
    val dataSize2 = shuffleClient.pushData(APP, 1, 0, 0, 0, DATA2, 1, 1)
    ...
    val dataSize3 = shuffleClient.mergeData(APP, 1, 0, 0, 0, DATA3, 1, 1);

    val STR4 = RandomStringUtils.random(16 * 1024)
    val DATA4 = STR4.getBytes(StandardCharsets.UTF_8)
    val LENGTH4 = DATA4.length
    val dataSize4 = shuffleClient.mergeData(APP, 1, 0, 0, 0, DATA4, 1, 1);
