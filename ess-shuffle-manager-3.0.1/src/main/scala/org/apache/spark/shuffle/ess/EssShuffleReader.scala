...
class EssShuffleReader[K, C](
  ...
  override def read(): Iterator[Product2[K, C]] = {
    ...
    val recordIter = (startPartition until endPartition).map(reduceId => {
      if (handle.asInstanceOf[EssShuffleHandle[_, _]].numMappers > 0) {
        val start = System.currentTimeMillis()
        val inputStream = essShuffleClient.readPartition(
          handle.asInstanceOf[EssShuffleHandle[_, _]].appId, handle.shuffleId, reduceId, context.attemptNumber())
